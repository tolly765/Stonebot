...
const { prefix, token, ignorechannel } = require('./config.json');
	...
	if (message.author.id != 66235899918159872) {
		if (!timestamps.haSs(message.author.id)) {
			...
			if (now < expirationTime) {
				...
				if (timeLeft > 1) {
					return message.reply(`Please wait ${timeLeft.toFixed(0)} more seconds before reusing the \`${command.name}\` command.`);
				}
			}
